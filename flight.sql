...
-- Hint: Look back at early practices where you were asked to find customers who bought one item and a second item.

-- SELECT m.Market_ID, Start_Airport, End_Airport 

/*WITH Q2_d AS (SELECT *
FROM market m
JOIN route r ON (m.Market_ID = r.Market_ID)
WHERE (Start_Airport = 'DEN' AND End_Airport ='BOI')), 
Q2_b AS (SELECT *
FROM market m
JOIN route r ON (m.Market_ID = r.Market_ID)
WHERE (End_Airport = 'DEN' OR Start_Airport ='BOI'));*/

/*Select * from (SELECT Start_Airport, End_Airport, Airline_ID
FROM market m
JOIN route r ON (m.Market_ID = r.Market_ID)
WHERE (Start_Airport = 'DEN' AND End_Airport ='BOI') 
OR (End_Airport = 'DEN' AND Start_Airport ='BOI'))tab
;*/
-- group by Airline_ID;


-- NEW TRY
SELECT Start_Airport, End_Airport, m.Market_ID, Airline_ID
FROM market m
JOIN route r ON (m.Market_ID = r.Market_ID)
...
FROM market m
JOIN route r ON (m.Market_ID = r.Market_ID)
WHERE ((End_Airport ='BOI') AND 
      ( Airline_ID = ANY (SELECT Airline_ID
							FROM market m
							JOIN route r ON (m.Market_ID = r.Market_ID)
							WHERE (Start_Airport = 'DEN'))))

order by End_Airport;
      
-- another new:

WITH Q2_eb AS(SELECT Start_Airport, End_Airport, market.Market_ID, Airline_ID
FROM market
JOIN route ON (market.Market_ID = route.Market_ID)
WHERE (End_Airport ='BOI')),
Q2_sd AS(SELECT Start_Airport, End_Airport, market.Market_ID, Airline_ID
FROM  market
JOIN route ON (market.Market_ID = route.Market_ID)
WHERE (Start_Airport = 'DEN'))
